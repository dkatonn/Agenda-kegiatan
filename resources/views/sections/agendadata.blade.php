<div class="agenda-box">
    ...
    <div class="agenda-item agenda-item-pinned">
        ...
        <div class="agenda-tag yellow">
            {{ $agenda->time }}
        </div>
    </div>
    @endforeach

    @if($agendaDataSlides->isNotEmpty())
    <div class="agenda-slider" style="height: calc((var(--agenda-row-height) * {{ max(1, $agendaDataRemainingSlots) }}) + 2px);">

        @foreach($agendaDataSlides as $index => $chunk)
        <div class="agenda-slide {{ $loop->first ? 'active' : '' }}" style="grid-template-rows: repeat({{ max(1, $agendaDataRemainingSlots) }}, var(--agenda-row-height));">
